...
const CONFIG = {
    ...
    // Direct API Configuration (for GitHub Pages deployment)
    DIRECT_API: {
        GEMINI: {
 ...
 * Tự động fallback sang direct API calls nếu proxy không khả dụng
 * @param {string} userInput - Input từ người dùng
...
async function callAIViaProxy(userInput, systemPrompt, provider, model) {
    ...
    const useProxy = await shouldUseProxyServer();
    
    if (!useProxy) {
        console.log('🌐 Using direct API call (no proxy available)');
        
        // Call direct API based on provider
        const enhancedUserInput = `Yêu cầu của người dùng: "${userInput}"\n\nHãy tạo một prompt hoàn chỉnh, chi tiết và chuyên nghiệp dựa trên yêu cầu này.`;
        
        if (provider === 'gemini') {
            return await callGeminiDirect(enhancedUserInput, systemPrompt);
        } else if (provider === 'openrouter') {
            return await callOpenRouterDirect(enhancedUserInput, systemPrompt, model);
        } else if (provider === 'groq') {
            return await callGroqDirect(enhancedUserInput, systemPrompt, model);
        } else {
            throw new Error(`Unknown provider: ${provider}`);
        }
    }
    ...
    try {
        ...
        if (error.message.includes('Failed to fetch') || error.message.includes('NetworkError')) {
            ...
            showToast('⚠️ Chuyển sang kết nối trực tiếp...', 'info', 2000);
...
/**
 * Call Gemini API directly (no proxy)
 */
async function callGeminiDirect(userInput, systemPrompt) {
    const url = `${CONFIG.DIRECT_API.GEMINI.API_URL}?key=${CONFIG.DIRECT_API.GEMINI.API_KEY}`;
...
/**
 * Call OpenRouter API directly (no proxy)
 */
async function callOpenRouterDirect(userInput, systemPrompt, model) {
    const providerConfig = CONFIG.AI_PROVIDERS.openrouter;
    const actualModel = providerConfig.models[model] ? 
        Object.keys(OPENROUTER_CONFIG.MODELS || {}).find(key => key === model) || model :
        model;
    ...
    const modelId = OPENROUTER_MODEL_MAP[model] || 'deepseek/deepseek-chat-v3.1:free';
    
    const response = await fetch(CONFIG.DIRECT_API.OPENROUTER.API_URL, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${CONFIG.DIRECT_API.OPENROUTER.API_KEY}`,
...
/**
 * Call Groq API directly (no proxy)
 */
async function callGroqDirect(userInput, systemPrompt, model) {
    ...
    const modelId = GROQ_MODEL_MAP[model] || 'llama-3.1-8b-instant';
    
    const response = await fetch(CONFIG.DIRECT_API.GROQ.API_URL, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${CONFIG.DIRECT_API.GROQ.API_KEY}`
        },
        body: JSON.stringify({
            model: modelId,
            messages: [
...
async function shouldUseProxyServer() {
    ...
    // Nếu config bắt buộc dùng proxy và đang chạy localhost
    if (CONFIG.USE_PROXY && CONFIG.isLocalhost) {
    ...
    // Nếu đang chạy trên production (GitHub Pages), không dùng proxy
    return false;
}
...
/**
 * Test connection tới AI provider - Direct API
 */
...
async function testAIConnection(provider = null) {
    ...
    try {
        ...
        if (!useProxy) {
            ...
            return await testProviderDirect(provider);
        }
        ...
        try {
            console.log('Trying direct API test as fallback...');
            return await testProviderDirect(provider);
        } catch (fallbackError) {
            return false;
        }
...
async function testAllAIConnections() {
    try {
        ...
        if (!useProxy) {
            console.log('🌐 Testing all providers with direct API calls');
            
            const results = {
                gemini: { success: await testProviderDirect('gemini'), provider: 'gemini' },
                openrouter: { success: await testProviderDirect('openrouter'), provider: 'openrouter' },
                groq: { success: await testProviderDirect('groq'), provider: 'groq' }
            };
        ...
        // Fallback to direct tests
        try {
            console.log('Trying direct API tests as fallback...');
            const results = {
                gemini: { success: await testProviderDirect('gemini'), provider: 'gemini' },
                openrouter: { success: await testProviderDirect('openrouter'), provider: 'openrouter' },
                groq: { success: await testProviderDirect('groq'), provider: 'groq' }
